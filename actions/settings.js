const Fields = '*name:String, *components:String, components2:String, *templates:String, token:String, darkmode:Boolean, backup:Boolean, notify:Boolean, *env:{dev|test|prod}, op_reqtoken:String, op_restoken:String, items:[*id:String, value:Object]'.toJSONSchema();
...
NEWACTION('Settings|read', {
	...
	action: function($) {
		...
		for (let key in Total.plugins) {
			...
			if (cfg) {
				let name = TRANSLATE(language, plugin.name);
...
NEWACTION('Settings|save', {
	...
	action: function($, model) {
		...
		let restartall = model.env !== PREF.env;
